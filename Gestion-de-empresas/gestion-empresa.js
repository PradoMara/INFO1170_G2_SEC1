...
document.getElementById('descargar-datos').addEventListener('click', () => {
    ...
    alert("Se descargarán los datos visibles en esta página de la paginación.");

    // Obtén los datos visibles en la página actual
    const inicio = (paginaActual - 1) * itemsPorPagina;
    const fin = inicio + itemsPorPagina;
    const empresasEnPagina = solicitudes.slice(inicio, fin);

    // Genera el contenido del archivo CSV
    const encabezados = ["ID", "Nombre de la Empresa", "RUT", "Dirección", "Correo Electrónico", "Documento", "Fecha"];
    const filas = empresasEnPagina.map(empresa => [
        empresa.id,
    ...
    // Combina encabezados y filas en un string CSV
    ...
    link.href = url;
    link.setAttribute('download', 'datos_paginacion.csv');
