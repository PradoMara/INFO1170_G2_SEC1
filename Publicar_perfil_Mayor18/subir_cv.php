...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['cv'])) {
    ...
    if ($archivo['error'] === UPLOAD_ERR_OK) {
        $nombreArchivo = basename(preg_replace("/[^A-Za-z0-9\.\-_]/", '', $archivo['name']));
        $rutaArchivo = $directorioDestino . $nombreArchivo; 

        if (move_uploaded_file($archivo['tmp_name'], $rutaArchivo)) {

            $sql = "INSERT INTO archivos_cv (nombre_archivo, ruta_archivo) VALUES (?, ?)";
            ...
            if ($stmt) {
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    echo "Error al ejecutar la consulta a la base de datos: " . mysqli_error($conexion);
                }
            } else {
